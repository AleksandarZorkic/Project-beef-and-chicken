...
namespace beef_and_chicken.Application.Interfaces.Repositories
{
    public interface IOrderRepository
    {
        ...
        Task<IEnumerable<Order>> GetActiveOrders(CancellationToken ct = default);
        ...
        Task<Order?> GetOrderByIdForUpdate(int orderId, CancellationToken ct = default);

        Task<IEnumerable<Order>> GetPendingOrders(CancellationToken ct = default);
        Task<IEnumerable<Order>> GetReadyForPickupOrders(CancellationToken ct = default);
        Task<IEnumerable<Order>> GetCourierOrders(int courierId, CancellationToken ct = default);

        Task<(List<Order> Items, int TotalCount)> GetOrderHistoryAsync(
            OrderHistoryQueryDto query,
            CancellationToken ct = default);

        Task<List<Order>> GetOrdersByIdsForUpdateAsync(
